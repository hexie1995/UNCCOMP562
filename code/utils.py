...
directory2 =  "C:\Users\hexie\Desktop\MURI_LINK\data\Stanford_dorm\miss"
...
def get_output_matrix(single_layer,k):
    ...
    with open(path+name+'_u_K'+str(k)+'.dat') as f:
        lines = (line for line in f if not line.startswith('#'))
        uk5 = np.loadtxt(lines, delimiter=' ', skiprows = 0 )
    uk5= uk5[:,1:]
    uk5 = np.array(uk5)
    
    with open(path+name+'_v_K'+str(k)+'.dat') as f:
        lines = (line for line in f if not line.startswith('#'))
    ...
    w5={}
    try:
        wk5 = pd.read_csv(path+name +'_w_K'+str(k)+'.dat', sep = " ")
        count = 0
        ...
        w5 = {}
        wk5 = pd.read_csv(path+name +'_w_K'+str(k)+'.dat',index_col=0 )
...
def read_double(data, layers):
    ...
    for item in pair:
        ...
        M = get_output_matrix(name)
        removed_edges = np.load(name+".npy")
...
def read_single_true(data, single,k):
    path="C:\Users\hexie\Desktop\MURI_LINK\MultiTensor-master\MultiTensor-master\data\\"
    name = str(single)
    print("layer is :", name)
    ...
    M = get_output_matrix(name,k)
    removed_edges = np.load(path+ name+".npy")
    removed_edges = zip ([int(x) for x in removed_edges[:,0]],[int(x) for x in (removed_edges[:,1])])
...
def read_double_true(data, double,k):
    path="C:\Users\hexie\Desktop\MURI_LINK\MultiTensor-master\MultiTensor-master\data\\"
...
def write_single(data, item):
    ...
    remove_count = int(round(len(all_edge_pair)*0.2))
...
def read_multiple_true(data, item,k):
    path="C:\Users\hexie\Desktop\MURI_LINK\MultiTensor-master\MultiTensor-master\data\\"
    name = ""
    for i in item:
        name = name + str(i)+"_"
